...
### Derse Özel Vurgu Rengi

Sayfa arka planı ve gövde metni her zaman siyah-beyaz kalır (bkz.
"Görsel stil"). Buna ek olarak, her ders için deterministik biçimde
üretilen TEK bir vurgu rengi kullanılır — aynı ders adı her seferinde
aynı rengi üretir (rastgele değil, ders adının karakterlerinden türetilen
sabit bir değerden hesaplanır — görsel PDF kitap sisteminin (build.py)
dinamik tema motoruyla aynı mantık).

Yeni bir ders işlenirken:
1. Ders adından (büyük harfli tam ad, örn. "SİSTEMATİK KELAM I")
   deterministik bir renk üret.
2. Üretilen rengin okunabilir/baskıya uygun olmasını sağla — çok açık
   (beyaza yakın) veya çok koyu (siyaha yakın) tonlardan kaçın, orta
   doygunlukta, kağıt üzerinde net ayrışan bir ton seç.
3. Bu rengi PDF üretimi tamamlanana kadar tüm bölümlerde tutarlı
   kullan.

#### Rengin uygulandığı öğeler (SADECE bunlar):

- **Bölüm/Bâb/Ana başlıklar** — başlık metni bu renkte, kalın
- **Kalın yazılan önemli kelime/terimler** — mevcut kuraldaki "kalın"
  vurgusu artık siyah değil, ders rengi olur (örn. anahtar terimler,
  isimler, kritik ifadeler)
- **Tablo başlık satırı** — tablonun ilk satırı (sütun başlıkları) bu
  renkte veya bu renkte arka planla

#### Rengin uygulanMADIĞI öğeler (siyah kalır):

- Gövde/açıklama metni
- Tercüme metni
- Arapça/orijinal dil metni
- Kaynak/Râvi bilgi satırı (bu ayrı, soluk gri bir stil kullanır —
  mevcut haliyle korunur)
- İtalik alıntılar
- 🔑 ⚠️ 💡 emoji başlıkları (bunlar kendi mevcut siyah/kalın stilini
  korur, ders rengine dönmez — böylece bu üç özel kutu türü tüm
  derslerde tanınabilir kalır)

#### Yeni vurgu katmanı: Altı Çizili Kritik Vurgu

Kalından ayrı, daha üst seviye bir vurgu katmanı olarak altı çizgi
eklenir. Bu, kalından daha nadir kullanılır — sadece gerçekten kritik,
mutlaka ezberlenmesi/atlanmaması gereken tek bir cümle veya terim için.
Örnekler: kaynak metinde "EZBERLENECEK" diye özellikle işaretlenmiş
hadisler/pasajlar, bir disiplinin en can alıcı tanımı, sınavda en çok
karıştırılan tek bir ayrım cümlesi.

- Altı çizili metin de ders rengiyle boyanır (kalın + altı çizili
  birlikte de olabilir, ders rengi ortak).
- Bu katmanı az ve öz kullan — sayfada altı çizili öğe sayısı kalın
  vurgulardan belirgin şekilde az olmalı, yoksa vurgu hiyerarşisi
  anlamını yitirir.

#### Teknik not — rengi ASLA elle seçme, `renk_uretici.py`'den al

Renk üretimi proje kökündeki **`renk_uretici.py`** modülünde tek bir
yerde tanımlıdır; görsel PDF kitap sistemi (`build.py`) ve bu skill AYNI
fonksiyonu kullanır, mantık hiçbir yerde kopyalanmaz. Rengi kendin
hesaplama, hex uydurma, `theme_engine`'i doğrudan çağırma:

```python
import sys
sys.path.insert(0, r"<proje kökü>")          # renk_uretici.py'nin bulunduğu klasör
from renk_uretici import ders_rengi_rgb
from docx.shared import RGBColor

RENK = RGBColor(*ders_rengi_rgb("HADİS", "2", "2", "final"))   # ders adı + dönem
run.font.color.rgb = RENK
run.bold = True
```

`ders_rengi()` / `ders_rengi_rgb()` şu sırayla çalışır:

1. O dönemin `src/*.py` dosyalarında `ders_klasoru=` alanı bu derse eşit
   olan bir ders modülü varsa, **görsel kitapta kullanılan renk** aynen
   alınır (`theme_color=` hex'i; LEGACY derslerde `theme="slate"` gibi
   sabit temanın `style.css`'teki `--accent` değeri). Yani görsel kitap
   sistemi otoritedir — Hadis dersi görsel kitapta hangi tonsa, anlatım
   PDF'inde de aynı tondur.
2. O ders için henüz bir `src/<ders>.py` yoksa renk, ders adından
   `hashlib` ile deterministik olarak türetilir ve görsel kitabın tema
   motorundan (`theme_engine.generate_theme_vars`) geçirilir — bu motor
   parlaklığı %26-39, doygunluğu okunur aralığa kelepçelediği için çıkan
   ton her zaman baskıya uygundur (yukarıdaki 2. maddenin gereği
   otomatik sağlanır).

Kontrol/teşhis için:

```bash
python renk_uretici.py "HADİS" --sinif 2 --donem 2 --sinav final   # rengi ve kaynağını yazar
python renk_uretici.py --liste --sinif 2 --donem 2 --sinav final   # dönemdeki tüm ders renkleri
```

`build.py` de her derlemede `[build] Ders vurgu rengi: #XXXXXX` satırını
basar; iki sistemin aynı rengi kullandığını buradan doğrulayabilirsin.

### Teknik yöntem

`python-docx` ile yukarıdaki stile uygun bir `.docx` oluştur, ardından `docx2pdf` paketiyle (Windows'ta Word'ü COM üzerinden kullanarak) PDF'e çevir. Bu yöntem, referans PDF'in zaten Word ile üretilmiş olmasından dolayı emoji ve fontların birebir aynı görünmesini garanti eder — özellikle emoji ikonları (🔑 ⚠️ 💡) düz PDF kütüphaneleriyle (reportlab, fpdf2 vb.) doğru render edilemeyebilir, Word/docx2pdf yolu bunu bypass eder. Gerekli paketler kurulu değilse `pip install python-docx docx2pdf --break-system-packages` ile kur. Word veya `docx2pdf` çalışmazsa yedek yöntem olarak `soffice --headless --convert-to pdf` (LibreOffice) kullanılabilir.
